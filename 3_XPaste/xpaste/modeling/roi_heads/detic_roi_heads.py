...
class DeticCascadeROIHeads(CascadeROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None,
        ...
        if self.training:
            ...
            has_gt_masks_id = torch.nonzero(proposals[0].objectness_logits.new_tensor([t.has('gt_masks') for t in targets]), as_tuple=True)[0]

            if ann_type == 'box' and targets[0].has('gt_masks') and len(has_gt_masks_id):
                features_ = {k: v[has_gt_masks_id] for k, v in features.items()}
                proposals_ = [proposals[i] for i in has_gt_masks_id]

                mask_losses = self._forward_mask(features_, proposals_, sem_seg_gt)
                losses.update({k: v * self.mask_weight \
                    for k, v in mask_losses.items()})
                losses.update(self._forward_keypoint(features_, proposals_))
            else:
