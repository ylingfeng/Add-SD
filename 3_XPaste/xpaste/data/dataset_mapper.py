...
class DatasetMapper:
    ...
    def from_config(cls, cfg, is_train: bool = True):
        ...
        ret = {
            ...
            "batch_sampling": cfg.DATALOADER.BATCH_SAMPLING,
        }
    ...
    def __call__(self, dataset_dict, augmentations=None):
        ...
        if "annotations" in dataset_dict:
            ...
            if not dataset_dict['instances'].has('gt_masks') and not self.batch_sampling:
                if self.instance_mask_format == "polygon":
                    # dataset_dict["instances"].gt_masks = PolygonMasks([[np.array(0,0,0,1,1,0)] for _ in range(len(dataset_dict["instances"]))])
                    pass
                else:
                    dataset_dict["instances"].gt_masks = BitMasks(np.zeros((len(dataset_dict["instances"]),image_shape[0],image_shape[1])))
            if not self.use_instance_mask and dataset_dict['instances'].has('gt_masks'):
                dataset_dict["instances"].remove("gt_masks")
        return dataset_dict
