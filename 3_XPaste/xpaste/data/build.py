...
def get_detection_dataset_dicts(
    ...
    for group_id, (dataset_name, dicts) in enumerate(zip(names, dataset_dicts)):
...
def build_batch_data_loader(
    ...
    **kwargs,
    ...
    if aspect_ratio_grouping:
        ...
        if batch_sampling:
            assert num_datasets > 1, "batch_sampling is only supported for num_datasets > 1"
            data_loader = GroupedAspectRatioGroupedDataset(data_loader, batch_size, num_datasets)
        else:
...
class GroupedAspectRatioGroupedDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        for d in self.dataset:
            ...
            bucket_id = d['group_id'] * 2 + aspect_ratio_bucket_id
