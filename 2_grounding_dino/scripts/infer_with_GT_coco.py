import json
from groundingdino.util.inference import load_model, load_image, predict, load_image_fast
import cv2
import os, sys
...
if __name__ == "__main__":
    ...
    for im_idx, info in tqdm(enumerate(datasets.data_infos[start:end])):
        ...
        if not getattr(pycocotools, '__version__', '0') >= '12.0.2':
            anns_ids = datasets.coco.getAnnIds(imgIds=[img_id]) # 该image id下的annotation id
        ...
        for gen_idx, gen_img_path in enumerate(gen_img_paths):
            ...
            for idx_, phr in enumerate(new_phrases):
                try:
                    phr_index = datasets.CLASSES.index(phr)
                except:
                    phr_index = datasets.CLASSES.index(ori_phrase)
                    new_phrases[idx_] = ori_phrase
